...
def prep_wav(orig_wav, out_wav, sr_override, wave_start, wave_end):
	...
	if (sr_models != None and SR not in sr_models) or (sr_override != None and SR != sr_override) or soxopts != "":
		...
		os.system("sox " + orig_wav + " -r " +
				  str(SR) + " " + out_wav + " " + soxopts)
	else:
...
def align(wav_file_path, transcript_file_path, output_file='output.TextGrid', sr_override=None, wave_start='0.0',
          wave_end=None, surround_token='sp', between_token='sp', mypath=None):
	wavfile = wav_file_path
	trsfile = transcript_file_path
	outfile = output_file
	...
	if sr_override != None and sr_models != None and not sr_override in sr_models:
		raise ValueError(
			"invalid sample rate: not an acoustic model available")
	...
	words = writeTextGrid(outfile, readAlignedMLF(
		output_mlf, SR, float(wave_start)))

	results = []
	for word in words:
		results.append({
			'word': word[0],
			'start_time': round(word[1], 3),
			'end_time': round(word[2], 3)
		})
...
if __name__ == '__main__':
	...
	print(align(wav_file_path=wavfile, transcript_file_path=trsfile, output_file=outfile, sr_override=sr_override,
				wave_start=wave_start, wave_end=wave_end, surround_token=surround_token, between_token=between_token,
				mypath=mypath))
